...
export class Game {
  ...
  applyGestures(gestureState) {
    ...
    if (gestureState.jogDetected) {
      this.jogBoost = Math.min(1, this.jogBoost + 0.05);
    } else {
      this.jogBoost = Math.max(0, this.jogBoost - 0.02);
    }
  }
  ...
  _update(dt) {
    ...
    this.speedMultiplier = 1 + this.jogBoost * 0.8;
